...
from .models import Customer,Admin,Patient
...
def viewezodiacsigns(request):
    ...
    zodiaclist = Patient.objects.all()
...
def displayzodiacsigns(request):
    ...
    zodiaclist = Patient.objects.filter(name__icontains=pname)
...
def addzodiac(request):
    ...
    if request.method == "POST":
        ...
        if formdata.is_valid():
            ...
            msg = "Failed to Add Patient Details"
            return render(request, "addzodiac.html", {"auname":auname,"productform": form, "msg": msg})
...
def deleteaps(request,aid):
    Patient.objects.filter(id=aid).delete()
